...
function () {
  ...
  _createClass(SnakeGame, [{
    ...
    value: function changeSnakePosition() {
      ...
      let snakeDirections = this.movement.split("|").map(function (direction) {
    ...
    value: function setBackground() {
      ...
      for (let i = 0; i < tileLengthInCanvasX; i++) {
        for (let j = 0; j < tileLengthInCanvasY; j++) {
    ...
    value: function drawSnake() {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        if (i === 0) {
          ...
          // body 
          let _pSeg = segments[i - 1];
          let _nSeg = segments[i + 1];
